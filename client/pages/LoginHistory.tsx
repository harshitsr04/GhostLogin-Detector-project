...
const LoginHistory = () => {
  ...
  const handleAllow = (rowId: number, location: string) => {
    setLoginData(loginData.filter((item) => item.id !== rowId));
    toast({
      title: "✓ Access Granted",
      description: `Login from ${location.replace(/\n/g, ", ")} has been allowed.`,
    });
  };

  const handleAccept = (rowId: number, location: string) => {
    setLoginData(loginData.filter((item) => item.id !== rowId));
    toast({
      title: "✓ Login Accepted",
      description: `Login from ${location.replace(/\n/g, ", ")} has been verified and accepted.`,
    });
  };

  const handleBlock = (rowId: number, location: string) => {
    setLoginData(loginData.filter((item) => item.id !== rowId));
    toast({
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-card border border-border rounded-lg p-4">
            ...
            <p className="text-3xl font-bold text-red-400 mt-2">
              {loginData.filter((d) => d.status === "Blocked").length}
            </p>
          </div>
          <div className="bg-card border border-border rounded-lg p-4">
            <p className="text-muted-foreground text-sm font-medium">
        ...
        {loginData.length > 0 ? (
          ...
          <div className="bg-card border border-border rounded-lg p-12 text-center">
            <p className="text-muted-foreground">No login records to display</p>
          </div>
        )}
      </div>
    </Layout>
